...
import com.dxj.service.GeneratorService;
...
public class GeneratorController {
    ...
    @ApiOperation("保存字段数据")
    @PutMapping
    public ResponseEntity<Void> save(@RequestBody List<ColumnInfo> columnInfos) {
    ...
    public ResponseEntity<Void> sync(@RequestBody List<String> tables) {
    ...
    public ResponseEntity<Object> generator(@PathVariable String tableName, @PathVariable Integer type, HttpServletRequest request, HttpServletResponse response) {
        if (!generatorEnabled && type == 0) {
            throw new SkException("此环境不允许生成代码，请选择预览或者下载查看！");
        }
        switch (type) {
            // 生成代码
            case 0:
