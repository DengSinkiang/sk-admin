...
public class QiNiuFileManage implements FileManage {
    ...
    @Autowired
    public QiNiuFileManage(SettingService settingService) {
    ...
    public OssSetting getOssSetting() {
        ...
        if (setting == null || StrUtil.isBlank(setting.getValue())) {
            throw new SkException("您还未配置七牛云对象存储");
        }
        return new Gson().fromJson(setting.getValue(), OssSetting.class);
    }

    private Configuration getConfiguration(int zone) {

        Configuration cfg;
        if (zone == SettingConstant.ZONE_ZERO) {
            cfg = new Configuration(Region.huadong());
        } else if (zone == SettingConstant.ZONE_ONE) {
            cfg = new Configuration(Region.huabei());
        } else if (zone == SettingConstant.ZONE_TWO) {
            cfg = new Configuration(Region.huanan());
        } else if (zone == SettingConstant.ZONE_THREE) {
            cfg = new Configuration(Region.beimei());
        } else if (zone == SettingConstant.ZONE_FOUR) {
